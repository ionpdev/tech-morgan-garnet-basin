  ...
  for (let i = 1; i <= 100; i++) {
    const application: TApplication = {
      ...
      loan_amount: parseInt(
        faker.finance.amount({ min: 1000, max: 150000, dec: 0 })
      ),
...
function fakeLoanHistory(): TLoanHistory[] {
  ...
  for (let index = 0; index < faker.number.int(10); index++) {
    ...
    const principle = faker.finance.amount({ min: 1000, max: 150000, dec: 0 })
    const interestRate = faker.finance.amount({ min: 1, max: 10, dec: 0 })
